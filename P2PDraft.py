...
connected_peers = set() # Set to store connected peers
last_mod_times = {} # track last modification times
...
def start_notification_listener():
    ...
    try:
        ...
        print(f"[!] Could not start notification listener on port {NOTIFY_PORT}: {e}")
    ...
    while True:
        ...
        for file in current_files - existing_files:
            ...
            if os.path.isfile(file_path):
                last_mod_times[file] = os.path.getmtime(file_path)
                log_event("LOCAL", file, "CREATED")

        # Detect deleted files
        for file in existing_files - current_files:
            last_mod_times.pop(file, None)
        ...
        for file in current_files & existing_files:
            ...
            if os.path.isfile(file_path):
                ...
                if last_mod_times.get(file) != current_mtime:
                    last_mod_times[file] = current_mtime
    ...
    while True:
        ...
        if command.startswith("get"):
            try:
                ...
                connected_peers.add(peer_ip) # Adds peer to notify list
                request_file(peer_ip, filename)
            except ValueError:
                print("Invalid input. Format: get <peer_ip> <filename>")
        else:
